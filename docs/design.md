...
#### 例1：シンプルな構造
基本的なブロック構造と固定回数の繰り返しを含む例です：

```json
{
  "protocolName": "センサーデータプロトコル",
  "structure": [
    {
      "name": "デバイスID",
      "type": "uint8"
...
このプロトコルでバイナリ`01 1234 50 60`をパースすると以下のように出力されます：
```
デバイスID : 01
センサー1
  温度 : 1234
...
#### 例2：可変長ブロック構造
ID参照による動的な繰り返し回数制御の例です：

```json
{
  "protocolName": "マルチセンサープロトコル",
  "structure": [
    {
      "id": "count",
      "name": "センサー数",
...
このプロトコルでバイナリ`02 01 1234 02 5678`をパースすると以下のように出力されます：
```
センサー数 : 02
センサーブロック(1)
  センサーID : 01
